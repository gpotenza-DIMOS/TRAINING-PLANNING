...
class GlobalTrainingPlanner:
    ...
    def analizza(self, sedi_config, r_km, d_pranzo, c_notte, v_uomo):
        ...
        for _, row in partecipanti.iterrows():
            ...
            if serve_pernotto: costo_pax += (c_notte * gg_training)
...
uploaded = st.file_uploader("Carica Excel con colonne 'citta', 'nome', 'ruolo'", type="xlsx")
if uploaded:
...
if not planner.df_pax.empty and st.session_state.sedi_custom:
    ...
    with col_a:
        ...
        res_a, map_a, tab_a = planner.analizza(config_a, r_km, 30, 130, v_uomo)
        if res_a:
    ...
    with col_b:
        ...
        res_b, map_b, tab_b = planner.analizza(config_b, r_km, 30, 130, v_uomo)
        if res_b:
    ...
    if map_a:
        ...
        st.pydeck_chart(pdk.Deck(
            ...
            layers=[
                pdk.Layer("ArcLayer", df_map, get_source_position=["lon_o", "lat_o"], get_target_position=["lon_d", "lat_d"], get_source_color=[0, 0, 255, 100], get_target_color=[0, 255, 0, 100], get_width=2),
